...
use baffao_core::token::{AccessToken, TokenManager};
use baffao_core::utils;

use crate::state::TmiState;

/// Session status response with access token
#[derive(Serialize)]
pub struct SessionStatusWithToken {
    ...
    T: TokenManager + 'static,
{
    ...
    let session_id = match utils::extract_session_id_from_cookie(
        ...
        None => return Ok(Json(SessionStatusWithToken {
            authenticated: false,
            user_id: None,
            access_token: None,
            expires_in: None,
    ...
    if session.is_expired() {
        return Ok(Json(SessionStatusWithToken {
            authenticated: false,
            user_id: None,
            access_token: None,
            expires_in: None,
    ...
    Ok(Json(SessionStatusWithToken {
        authenticated: true,
        user_id: Some(session.user_id.clone()),
    ...
    T: TokenManager + 'static,
{
    ...
    // Check if we have a valid access token for these scopes
    ...
    // Try to get a refresh token to get a new access token
    let refresh_token = match state.token_manager.get_refresh_token(&session.user_id).await? {
        Some(token) => token,
        None => return Err(BaffaoErrorResponse(BaffaoError::Unauthorized)),
    };
    
    // Get a new access token with the requested scopes
    let auth_response = state.oauth_client
        .refresh_token(&refresh_token.token)
        .await?;
